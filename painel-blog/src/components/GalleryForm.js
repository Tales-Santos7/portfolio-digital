...
const GalleryForm = () => {
  ...
  const handleRemove = async (imageUrl) => {
    ...
    try {
      const response = await axios.delete(`${baseUrl}/content/gallery?imageUrl=${encodeURIComponent(imageUrl)}`);
      setImages(response.data.images);
  ...
  const handleUpdateSubmit = async (e, imageUrl) => {
    ...
    try {
      const response = await axios.put(`${baseUrl}/content/gallery/update`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
  ...
  return (
    <div className="card-form">
      ...
      <form onSubmit={handleUpload}>
        ...
        <button className="btn-blue margin-botton" type="submit">Adicionar Imagens</button>
      </form>
      ...
      <div className="gallery" id="galeria-container">
        {images.map((url, index) => (
          <div key={index} className="gallery-item">
            ...
            <button className="btn-red margin-btn-galery" onClick={() => handleRemove(url)}>Remover</button>
            <button className="btn-green" onClick={() => handleUpdateClick(index)}>Atualizar</button>

            {updateImageIndex === index && (
              <form onSubmit={(e) => handleUpdateSubmit(e, url)} style={{ marginTop: '10px' }}>
                <input type="file" onChange={handleUpdateFileChange} />
                <button className="btn-blue margin" type="submit">Enviar Nova</button>
                <button
                  type="button"
